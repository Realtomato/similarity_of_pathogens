...
def get_reads_by_step(seq_names, seqs, step, outf):
	...
	for j in range(0, len(seqs)-39, step):
		outf.write('{0}_{1}\n'.format(seq_name, count))
		if len(seqs[j:j+40])<40:
			outf.write(seqs[(j-10):(j+40)] + '\n')
			break
		outf.write(seqs[j:j+40] + '\n')
		count += 1
	outf.close()
# def get_reads_by_step(seq_names, seqs, step, outf):
# 	count = 0
...
def get_reads_by_random(seq_names, seqs, ScoreOfScaffolds, outf):
	...
	for i in range(len(seq_names)):
		...
		seq = seqs[i]
		# print(len(seq)-39)
		random_idx = np.random.choice(len(seq)-39, ScoreOfScaffolds[i], replace=False)
...
if TotalBases - 39 * len(LenOfScaffolds) >= 40*100*1000:
	step = int(TotalBases/(100*1000))+1
	print(TotalBases)
	print(step)
	get_reads_by_step(seq_names, seqs, step, outf)
